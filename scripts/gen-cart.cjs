...
export function Cart() {
  ...
  const belowMinimum = settings ? totals.subtotal < settings.minOrderTotal : false;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-extrabold text-slate-900">
          Cart <span className="text-sm font-semibold text-slate-500">({itemCount} items)</span>
        </h1>
        <button onClick={clear} className="text-sm font-semibold text-red-700 hover:underline">
          Clear all
        </button>
      </div>
            ...
            <Card key={line.productId} className="flex gap-3">
              ...
              <div className="flex flex-1 flex-col gap-1.5">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button onClick={() => remove(line.productId)} className="text-xs font-semibold text-red-700 hover:underline" aria-label={`Remove ${line.name}`}>
                    Remove
                  </button>
                </div>
                <p className="text-xs text-slate-500">{formatMoney(line.unitPrice)} each</p>
                <input
                  value={line.notes ?? ''}
                ...
                <div className="mt-auto flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button onClick={() => setQuantity(line.productId, line.quantity - 1)} className="h-9 w-9 rounded-xl bg-slate-100 font-bold text-slate-800" aria-label="Decrease">
                      −
                    </button>
                    <span className="w-6 text-center font-extrabold text-slate-900">{line.quantity}</span>
                    <button onClick={() => setQuantity(line.productId, line.quantity + 1)} className="h-9 w-9 rounded-xl bg-slate-100 font-bold text-slate-800" aria-label="Increase">
                      +
                    </button>
                  </div>
